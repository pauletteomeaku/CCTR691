...
> <1. we counted about 5,000 but there is oveer 17,000. Meaning there needs to be a quality check because this number is way too high. 2. The TSNE is not breaking out in discrete group but showing up more uniformly>
...
```{r setup}
knitr::opts_knit$set(root.dir = "/lustre/home/omeakupb/git_repos/CCTR691/CCTR691_scRNASeq")
```
```{r}
#install.packages(c("SeuratObject","Matrix","dplyr","ggplot2"))
...
> <original identifier, RNA count, RNA feature count>
...
> <19,612 features number of gene names; 2,575 are the number cells >
...
> <The majority of cells cluster below 20% mitochondrial, but a tail extends upward toward 100%, which is typical of low-quality cells.>
...
whim_filtered <- subset(whim, subset = percent.mt <20)
...
> <The cells that did not survive were likely of poor quality (low feature and RNA counts and high mitochondrial numbers or expression. > 
...
> <The x-axis represents cells, and the y-axis represents the top genes that contribute the most to each PC.
Each heatmap shows how expression levels of these genes vary across the cells, and the color intensity shows scaled expression.>
...
> <As the PC number increases, the heatmaps show less unique structure and more noise.
The first few PCs capture the most significant variation in the data. There are clear gene expression patterns and separations between groups of cells.>
...
> <The tSNE plot shows less of the variability in the dataset. The plot shows incomplete clusters and relies on a smaller portion of the total variance.>
...
> <Using the first PC drastically simplifies the data and becomes linear. There's no clear way to define or see clustering>
...
> <The tSNE shows more variation and clearer structure in the data.>
...
> <Cluster 1 (green) had the strongest expression for both SCGB2A2 and KRT16>
...
> <The x-axis represents individual cells, grouped by their assigned Seurat clusters>

Q15) Which 2 clusters have the most distinct expression profile from the rest of the clusters?

> <Clusters 1 and 2 show the most distinct expression>
...
> <The log2 fold change of SCGB2A2 is  2.97, meaning there is a strong upregulation in Cluster 1 compared to all other clusters>
